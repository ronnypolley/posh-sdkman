...
Describe 'Set-Env-Candidate-Version' {
    Context 'Env-Version of grails is current' {
        ...
        It 'sets GRAILS_HOME' {
            $env:GRAILS_HOME | Should -Be (Join-Path -Path $Global:PSDK_DIR -ChildPath "grails" -AdditionalChildPath "1.3.7")
        }

        It 'extends the Path' {
            ($env:Path -split ";") | Should -Contain (Join-Path -Path $Global:PSDK_DIR -ChildPath "grails" -AdditionalChildPath "1.3.7", "bin")
        }
     ...
     Context 'Tool should not be added to $env:Path if not installed' {
        ...
        It 'sets GRAILS_HOME' {
            $env:GRAILS_HOME | Should -BeExactly (Join-Path -Path $Global:PSDK_DIR -ChildPath "grails" -AdditionalChildPath "1.3.7")
        }

        It 'not extends the Path' {
            ($env:Path -split ";") | Should -Not -Contain (Join-Path -Path $Global:PSDK_DIR -ChildPath "grails" -AdditionalChildPath  "1.3.7", "bin")
        }
